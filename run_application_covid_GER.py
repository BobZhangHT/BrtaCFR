...
FIT_EXTEND_DAYS = 30  # Extend fitting by 30 days to reduce tail effects
...
def extract_country_timeseries(df, country_name, start_date, end_date):
    ...
    # Create a DataFrame
    ts_df = pd.DataFrame({"date": dates, "cumulative": values})
...
def compute_daily_new(cumulative_data):
    ...
    """
    daily = cumulative_data.diff().fillna(
        cumulative_data.iloc[0] if len(cumulative_data) > 0 else 0
    )
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="BrtaCFR real-data application: Germany")
    parser.add_argument(
        "--lambda_scale",
        type=float,
        default=1.0,
    ...
    print(
        f"Fitting date range: {START_DATE} to {extended_end_date} (extended by {FIT_EXTEND_DAYS} days)"
    )
    ...
    print(
        f"  Overall crude CFR: {(cumulative_deaths.iloc[-1]/cumulative_confirmed.iloc[-1]*100):.2f}%"
    )
    ...
    if results.get("lambda_draws") is not None:
        ...
        lambda_summary_path = Path(output_dir) / "covid_germany_lambda_summary.csv"
    ...
    _style_cfr_ax(ax)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "covid_germany_cfr_curves_smooth.pdf"), dpi=300)
    print(f"✓ Saved CFR plot (smooth) to {output_dir}/covid_germany_cfr_curves_smooth.pdf")
    plt.close()
    ...
    _style_cfr_ax(ax)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "covid_germany_cfr_curves_raw.pdf"), dpi=300)
    print(f"✓ Saved CFR plot (raw) to {output_dir}/covid_germany_cfr_curves_raw.pdf")
    plt.close()
    ...
    print("  - covid_germany_lambda_summary.csv (posterior lambda: median, q025, q975)")
    print("  - covid_germany_cfr_curves_smooth.pdf")
    print("  - covid_germany_cfr_curves_raw.pdf")
